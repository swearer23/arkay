...
const { writeJsonSync, readJsonSync, readFileSync, writeFileSync, readdirSync } = fsExtra;
...
const postTemplateInit = (localPath, name) => {
  ...
  let componentPages = readdirSync(localPath+'/src'), commands = [];
  componentPages.forEach(page=>{
    if(page.indexOf('Component')>-1){
      let newPage = page.replace('Component',camelName)
      commands.push(`mv ${localPath}/src/${page} ${localPath}/src/${newPage}`)
    }
  })
  commands.push(`mv ${storyFilePath} ${storyNameFilePath} `)
