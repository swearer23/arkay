...
export const add = async name => {
  try {
    ...
    log.info('==> you are ready to set, now get into the component folder and play')
...
export const clone = name => {
  ...
  if (pathExistsSync(`./components/${name}`)) {
    log.error('==> folder with same name already exists, try another project name')
...
export const newMergeRequest = async name => {
  if (await prompt.confirmNewMr()) {
    try {
      ...
      // if(error) return console.log(chalk.red(message))
      log.info("==> merge request was successfully created。")
      log.info(`==> you can see at: ${merge_request_url}`)
      log.info("==> thanks for contribution！")
      // console.log(chalk.green("==> merge request was successfully created。"))
      // console.log(chalk.green(`==> you can see at: ${merge_request_url}`))
      // console.log(chalk.green("==> thanks for contribution！"))
    } catch (err) {
      log.error(err)
      log.error("==> it seems your are not the member of this project，Ask the Owner to open the permission first, and then try again")
      // console.log(chalk.red(err))
      // console.log(chalk.red("==> it seems your are not the member of this project，Ask the Owner to open the permission first, and then try again"))
    }
  }
}
