...
export const release = (name, upgradeMode) => {
  ...
  log.combine('ERROR[==> ]', `ERROR[git repo has uncommited changes, please commit or stash before release]`)
...
export const publish = (name) => {
  ...
  if (pathExistsSync(COMPONENT_PATH)){
    if (checkIfRepoUncommited(COMPONENT_PATH)) {
      log.error('Changes have not been added to commit (use "git add" and/or "git commit -a")')
    ...
    .then(answers => {
      ...
      if(!tag){
        ...
        .then(async answers => {
          ...
          if(userInput == 'n'){
            log.error("==> thanks for contribution")
            return
          }else if(userInput == 'y'){
            inquirer.prompt([
              {
