...
import { COMPONENT_TEMPLATE_PATH, COMPONENT_TEMPLATE_TAG } from '../consts.js';
...
const lockPackageJson = packagePath => {
  execSync(`cd ${packagePath} && npx pnpm install --lockfile-only && git add .`, {stdio:[0,1,2]})
}
...
export const tinyDegit = targetDir => {
  const commands = [
    `mkdir -p ${targetDir}`,
    `cd ${targetDir}`,
    `git clone --branch  ${COMPONENT_TEMPLATE_TAG} --depth 1 ${COMPONENT_TEMPLATE_PATH} .`,
    `rm -rf .git`
  ]
